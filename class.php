...
class ExamProces {
  ...
  public function coompare_answers($student_answers) {
    ...
    */
    foreach ($this->compare_table as $index => $question):
        ...
        endforeach;
        $this->compare_table[$index][1] = $question[1];// assign "Q_mark" in compare_table LINKE => Q_id => [question_obj, Q_mark]
    endforeach;
  }
  ...
  public function some_markes() {
    ...
    $markes = [];
    foreach ($this->compare_table as $index => $question):
      $question_mark = (100 / count($this->compare_table)) * ($question[1] / 100);
      $full_marke += $question_mark;
      array_push($markes, [$question[0]->id, $question[0]->answers, $question_mark]);
    endforeach;
